"use client";

import { useTheme } from "next-themes";
import { useEffect, useState, useRef } from "react";

export function ThemeTransition() {
  const { theme } = useTheme();
  const [mounted, setMounted] = useState(false);
  const prevThemeRef = useRef<string | null>(null);

  useEffect(() => {
    setMounted(true);
  }, []);

  useEffect(() => {
    if (!mounted || !theme) return;

    // 首次加载时设置 prevTheme
    if (prevThemeRef.current === null) {
      prevThemeRef.current = theme;
      return;
    }

    // 如果主题没变，不执行动画
    if (prevThemeRef.current === theme) return;

    // 记录变化前的旧主题
    const oldTheme = prevThemeRef.current;

    // 浅色 → 深色：黑色展开
    // 深色 → 浅色：白色展开
    const bgColor = oldTheme === "light" ? "#171717" : "#ffffff";
    const textColor = oldTheme === "light" ? "#ffffff" : "#171717";

    // 更新旧主题记录
    prevThemeRef.current = theme;

    // 创建动画覆盖层
    const overlay = document.createElement("div");
    overlay.id = "theme-transition-overlay";
    overlay.style.cssText = `
      position: fixed;
      top: 0;
      left: 0;
      width: 100%;
      height: 100%;
      background: ${bgColor};
      pointer-events: none;
      z-index: 9999;
      display: flex;
      flex-direction: column;
      align-items: center;
      justify-content: center;
      animation: themeExpand 0.7s ease-out forwards;
    `;

    // 创建图标
    const icon = document.createElement("img");
    icon.src = "/star.ico";
    icon.style.cssText = `
      width: 80px;
      height: 80px;
      margin-bottom: 20px;
      animation: fadeIn 0.3s ease-out;
    `;

    // 创建 LLTTH 文字
    const text = document.createElement("div");
    text.textContent = "LLTTH";
    text.style.cssText = `
      font-family: 'Courier New', monospace;
      font-size: 48px;
      font-weight: bold;
      letter-spacing: 12px;
      color: ${textColor};
      text-shadow: 0 0 20px ${textColor}80;
      animation: fadeIn 0.3s ease-out, glow 1.5s ease-in-out infinite alternate;
    `;

    // 添加动画关键帧
    if (!document.getElementById("theme-anim-style")) {
      const style = document.createElement("style");
      style.id = "theme-anim-style";
      style.textContent = `
        @keyframes themeExpand {
          0% {
            clip-path: circle(0% at 100% 0%);
          }
          100% {
            clip-path: circle(150% at 100% 0%);
          }
        }
        @keyframes fadeIn {
          from { opacity: 0; transform: scale(0.8); }
          to { opacity: 1; transform: scale(1); }
        }
        @keyframes glow {
          from { text-shadow: 0 0 20px ${textColor}80, 0 0 40px ${textColor}40; }
          to { text-shadow: 0 0 30px ${textColor}aa, 0 0 60px ${textColor}60; }
        }
      `;
      document.head.appendChild(style);
    }

    overlay.appendChild(icon);
    overlay.appendChild(text);
    document.body.appendChild(overlay);

    // 动画完成后移除覆盖层
    setTimeout(() => {
      overlay.remove();
    }, 700);
  }, [theme, mounted]);

  return null;
}
